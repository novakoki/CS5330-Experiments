...
class PointPillars(nn.Module):
    ...
    def voxelize(self, points_list: List[torch.Tensor]):
        ...
        for batch_idx, points in enumerate(points_list):
            ...
            batch_indices = torch.full(
                (coords.shape[0], 1), batch_idx, dtype=torch.int64, device=coords.device
            )
    ...
    def forward(self, batch: Dict) -> Dict[str, torch.Tensor]:
        ...
        points_on_device = [p.to(device, non_blocking=True) for p in batch["points"]]
        voxels, coords, num_points, batch_size = self.voxelize(points_on_device)
